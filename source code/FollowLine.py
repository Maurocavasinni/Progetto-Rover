...
def FollowLine():
	if (GPIO.input(IR_L) == False)&(GPIO.input(IR_R) == False):# Black line is high, ground is low
		...
		Motor_TurnLeft()
		return
	elif (GPIO.input(IR_L) == True)&(GPIO.input(IR_R) == True):# Both of the two side connect to the black line
		Motor_Stop()
		return
...
IR_R = 18	#Right side of the car line inspection infrared
IR_L = 27	#Left side of the car line inspection infrared
